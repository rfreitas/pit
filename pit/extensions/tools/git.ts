...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    promptSnippet:
      "Run git commands (commit, diff, log, status, etc.) in the worktree",
    parameters: Type.Object({
      args: Type.Array(Type.String(), {
        description:
          'git arguments, e.g. ["commit", "-m", "fix: update foo"] or ["status"] or ["log", "--oneline", "-5"]',
      }),
    }),
    async execute(_id, params, signal) {
      ...
      return Effect.runPromise(
        Effect.gen(function* () {
          ...
          if ("error" in resp) {
            return {
              content: [{ type: "text" as const, text: resp.error }],
              isError: true,
              details: { code: undefined as number | undefined },
            };
          }

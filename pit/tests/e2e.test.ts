...
describe("pit -r — inner pit resumes existing session", () => {
  ...
  it.skipIf(!hasBwrap)(
    "symlinked agentDir works correctly and resumes existing session",
    () => {
      const repo     = makeGitRepo(tmpDirs);
      
      // Create a REAL dir, and a SYMLINK to it.
      const realAgentDir = makeAgentDir(tmpDirs);
      const symlinkAgentDir = realAgentDir + "-symlink";
      fs.symlinkSync(realAgentDir, symlinkAgentDir, "dir");
      tmpDirs.push(symlinkAgentDir);

      const agentDir = symlinkAgentDir; // Use the symlink!
      ...
      const SENTINEL_ID = "symlink-sentinel";
      fs.appendFileSync(sessionFile, JSON.stringify({
        type: "user", id: SENTINEL_ID, parentId: null,
        timestamp: new Date().toISOString(),
        content: [{ type: "text", text: "sentinel for symlink test" }],
      }) + "\n");

      // 2. Run pit -r using the test-only picker bypass, from inside the worktree
      const resumeRun = runPit(
        ["-r", "--mode", "json"],
        {
          ...
          agentDir,
          extraEnv: {
            PIT_TEST_PICKER_SESSION: sessionFile,
          }
        }
      );

      const filesAfter = fs.readdirSync(sessionDir).filter(f => f.endsWith(".jsonl"));
      expect(filesAfter.length, `Failed with symlink!\nstderr: ${resumeRun.stderr?.slice(0, 1000)}`).toBe(1);
